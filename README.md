...
- **SSH** normally uses only the **private key** paired with `SSH_PUBKEY_PATH` (e.g. `ssh -i ~/.ssh/id_ed25519 pi@10.42.0.1`). If you see *“SSH may not work until a valid user has been set up”* and **publickey** keeps failing, first-boot may not have installed your key — run **`ssh -vvv …`** to confirm “Offering public key” / “Permission denied”. For a one-time rescue, set **`SSH_PASSWORD_AUTH=1`** in `config.env`, **`make flash` again**, then SSH with the password in **`.last_password`** (copy with **`pbcopy < .last_password`** on macOS so you do not paste an extra newline); confirm on the boot volume that **`custom.toml`** contains **`password_authentication = true`**. **`./03-bootstrap.sh`** disables password auth again afterward.
